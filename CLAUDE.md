...
**Backfilling a field added after documents already exist is not optional.** Adding a field to a TypeScript type does nothing to documents already sitting in Firestore — they keep whatever shape they had when they were last written, and reading them back as the new type just gives `undefined` for the field that doesn't exist yet on that particular doc. This bit twice in the same day: `company.verification` reads were hardened with optional chaining *and* every existing company was migrated to an explicit value (§11); `applicantProfiles.basicInfo.address`/`.about` and `jobs.bannerUrl` were caught the same way after a live check found 33/33 profiles and 34/37 jobs missing the new fields entirely — and confirmed, by directly reproducing it, that saving an edit to any of them would crash: every edit action here rebuilds the full document with `{...existing, ...input}` before `.set()`, and the Firestore Admin SDK throws `Cannot use "undefined" as a Firestore value` on that by default. Fixed two ways, not just one — `adminDb()` (`lib/firebase/admin.ts`) now sets `ignoreUndefinedProperties: true` globally, so this exact bug class can't recur for any field, present or future, and every existing document was also explicitly backfilled to the new shape (`address: ""`, `about: ""`, `bannerUrl: null`) so reads are consistent rather than relying on `undefined`-is-falsy in every consumer. When adding a new field to an existing collection: assume live documents don't have it, and either backfill them or make every read site defensive (ideally both).
...
*(Deviation, 2026-08-21: expanded again to 20 companies / 48 jobs / 30 applicants, explicit request for a rounder, larger, industry-diverse dataset (adding software, BPO, design, logistics, healthcare, real estate, legal, and HR/recruitment companies). Score spread across the resulting 1,440 matches (30 applicants × 48 open jobs) is not clustered at either extreme — roughly 22% score 0–20, 65% score 21–40, 10% score 41–60, and the remaining ~3% score 61–100 — satisfying this section's "not everything clustered at 0% or 100%" goal. This same date, all real (non-`@jobhunt.test`) accounts were purged from production: 7 accounts across 5 known emails plus 2 more discovered only by cross-checking Firebase Auth's user list directly against Firestore, since two of them had no `users` doc at all (the email-suffix check that finds every other real account can't see an account with no Firestore doc to check) — see §15 for the general lesson. Everything in the database as of this date is script-generated; verified directly via `auth.listUsers()` cross-referenced against every `applicantProfiles`/`companies`/`applications`/`matches` doc, not just the `users` collection.)*
...
- Adding a field to a Firestore-backed TypeScript type without backfilling documents that already exist — see §4's dated note. The type checker has no idea what's actually sitting in the database; a document written before the field existed reads back as `undefined` for it, and the default Firestore Admin SDK behavior is to throw on writing `undefined` back, not silently drop it. This is a real crash on every edit of an old document, not a theoretical one.
- Identifying "real" vs. seeded accounts by scanning the `users` Firestore collection alone — a `users/{uid}` doc can be missing entirely (deleted separately from the rest of that account's data, or never fully written) while `applicantProfiles`, `companies`, `applications`, and `matches` docs for that same uid still exist. Cross-check against the authoritative list from `auth.listUsers()` instead when you actually need to know every account that exists — see §10 Phase 1.5's dated note for a case where this exact gap let two stray accounts survive a first cleanup pass.
